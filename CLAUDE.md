...
- **主题（v2）**: 换肤机制 = 每主题覆盖 `--color-gray-*`（中性色）与 `--color-indigo-*`（品牌色）等 CSS 变量（`index.css`），组件类名不变。现代组：`dark`（默认，Multica 风 zinc 中性色 + 蓝品牌色，oklch）、`light`（中性色反转 + accent 300/400 档反转成深色调保对比度）、`feishu`（飞书 Universe Design 官方色板：N 系中性色 + 品牌蓝 B500 #336df4，hover/pressed 向深走 B600/B700，功能色用飞书 R/O/G/P/B）；Legacy 组：`legacy`（v1 默认外观，Tailwind 原生 gray/indigo）、`ocean`/`forest`/`rose`。**新增主题必须同时覆盖 gray 全档 + indigo 全档**（`frontend/src/config/theme.test.ts` 有自动化断言）；浅色兼容规则：中性底上禁用 `text-white`/`hover:text-white`，一律 `text-foreground`（彩色实底按钮除外）。字体 Inter Variable + JetBrains Mono（@fontsource-variable，随 bundle 离线）。App 壳 `AppShell.tsx`：桌面 lg+ 固定侧栏 + sticky 顶栏（h-12 + 1px 边框，TasksPage 分屏高度按 `100vh-49px` 计算），移动端抽屉导航
