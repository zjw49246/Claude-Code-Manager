...
- **Dispatcher instance 预留原子性**: 所有本地 instance 分配路径（新 task + 不同 task 的 queued-message consumer）必须经 `_instance_claim_lock` 在同一临界区完成 idle 选择与 `_launching_instances` 预留；预留用 token/owner 校验释放，避免旧 cleanup 误删新 claim。`_running_tasks` 同时含本地整数 instance ID 与 `worker-{task_id}` 字符串键，构造 `Instance.id` SQL 过滤条件时必须只取整数键（PostgreSQL 不接受字符串绑定到 integer）。底层仍报 `InstanceAlreadyRunningError` 时原 `QueuedMessage` 必须重排队，绝不能丢用户消息。
