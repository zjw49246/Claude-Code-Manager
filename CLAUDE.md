...
│       ├── mcp_config.py        # Provider-neutral MCP specs + Claude config renderer
...
- **账号登录浏览器运行时**: Claude/Codex Pool 必须共用 `backend/services/login_runtime.py` 的进程内登录锁与 Xvfb manager，禁止各模块缓存/`pkill` Xvfb 或 Chrome。Xvfb 使用私有 Xauthority、跨进程 display 文件锁与 `xdpyinfo` 就绪探测；只可终止 manager 自己持有的子进程。多套 CCM 同机部署必须分别配置 `CCM_XVFB_DISPLAY` 与 `CCM_LOGIN_CDP_PORT`（例如生产 `:99/9222`、测试 `:100/9223`）。浏览器 profile 统一写 `CCM_LOGIN_TMPDIR` 的磁盘目录；启动 Chrome 前按 `CCM_LOGIN_MIN_AVAILABLE_MB`/`CCM_LOGIN_MIN_TEMP_FREE_MB` fail-fast，Codex authorize 导航只允许一次诊断性重试。
...
- **Skills 系统**: Task.enabled_skills（JSON dict，如 `{"monitor": true}`）控制注入哪些 MCP 工具。创建 task 时勾选 Skills，dispatcher 根据 enabled_skills 动态生成 MCP config 并通过 `--mcp-config` 注入 Claude CLI。聊天页的任务级 Distill 必须按 `Task.provider` 分流；Codex 使用绑定/健康账号运行独立 `codex exec --ephemeral`，不得复用或改写原 task 的 thread/账号绑定。该临时进程必须通过 `InstanceManager.codex_home_exec_guard` 纳入同一 CODEX_HOME maintenance 准入，取消/超时须 kill + reap 后才释放 home；对话属于不可信输入，Codex distill 必须使用只读 sandbox 并忽略用户 config/rules，严禁 `--dangerously-bypass-approvals-and-sandbox`
...
- **MCP 架构**: `mcp_config.py` 先为主任务、Monitor、Sub-Agent 构造 provider-neutral `McpServerSpec`（command/args/cwd/env/required/enabled_tools/timeouts），再由 Claude renderer 生成兼容的 `mcpServers` JSON；当前生产注入仍仅走 Claude CLI。三个 FastMCP server 分别是 `ccm_skills_server.py`、`ccm_monitor_agent_server.py`、`ccm_sub_agent_server.py`，通过 stdio 与 CLI 通信、通过 HTTP 调用 CCM API。配置文件动态生成到临时目录并在对应进程结束后清理
...
- **PR Monitor**: GitHub PR 自动审核功能。GitHub Webhook 推送 PR 事件 → 创建 CCM task 让 Claude 审核 → 审核通过可自动 merge。`synchronize` 替换旧审核时必须先经 `task_termination.terminate_authoritative_task_generation` 安全终止旧 generation：本地清空/中止 queue，以 Task CAS + 同事务 Instance `(id,pid,started_at)` 快照精确 stop；Worker 则持有 migration/operation locks，GET 校验 remote retry generation 后调用隐藏的 full-shield termination endpoint，再 authoritative CAS Manager mirror。两路都要确认无残留并锁定 exact resulting Task generation直至旧 Review superseded + replacement commit；失败一律 409，不得误杀复用 slot 或宣告新审核成功。数据模型：`MonitoredRepo`（监控仓库配置）+ `PRReview`（审核记录）。Webhook 端点 `/api/github/webhook`（公开，HMAC-SHA256 验签）。前端独立页面 `PRMonitorPage`。Webhook URL: `https://youchengsong.claude-code-manager.com/api/github/webhook`
...
- **认证**: 除 `/api/system/health`、`/api/auth/login`、`/api/github/webhook` 外，所有 API 需要 `Authorization: Bearer <token>`。JWT 的 role/is_active 必须每个 HTTP 请求以 User 当前数据库行为准，删除/禁用立即 401、降级立即失去 admin 权限；`/api/instances`、`/api/dispatcher` 与 system update/status 的读写，以及 system skill curator/distill 的触发均 admin-only。`POST /api/ask-user/wait` 是内部 hook 入口，启用 AUTH_TOKEN 时只接受 legacy service token，不能接受用户 JWT。
