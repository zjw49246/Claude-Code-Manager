...
- **Codex provider 对等逻辑**: Task/Instance 的 `provider` 字段（claude/codex）分流所有 CLI 相关行为。Codex 侧的指令文件是 **AGENTS.md**：① project 创建（clone/init）时 `_inject_agents_md`（`backend/api/projects.py`）注入指向 CLAUDE.md 的 symlink（无 symlink 权限的平台回退 pointer 文件）并随 CLAUDE.md 一起 commit；② dispatcher 的所有 prompt（task/goal/loop）经 `_agent_doc_preamble`/`_agent_doc_name` 按 provider 引用 AGENTS.md（老项目无 AGENTS.md，codex 措辞带 CLAUDE.md 回退）；③ skills 模板只对 claude 注入（MCP config 仅 claude CLI 支持，codex 收到只会调不存在的工具）。claude-only 的能力（MCP/PTY/pool/thinking budget/ask_user hook）在 instance_manager.launch 已按 provider 门控
...
- **CLAUDE.md**（本文件）：架构、约定、关键路径变化时更新，只改变化的部分，保持简洁。**AGENTS.md 是指向本文件的 symlink**（Codex CLI 读取 AGENTS.md），单一事实源，不要把它改成独立文件
