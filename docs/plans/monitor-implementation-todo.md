...
- [x] 新建 `backend/models/monitor_session.py`
...
- [x] 在 `alembic/env.py` 中 import MonitorSession, MonitorCheck（项目的 model 注册在这里，不是 `__init__.py`）

### 1.3 创建 Alembic Migration
- [x] `alembic revision --autogenerate -m "add_monitor_sessions_and_checks_tables"`
- [x] 检查生成的 migration 文件，确认表结构正确
- [x] `alembic upgrade head` 验证迁移成功

### 1.4 创建 Schema
- [x] 新建 `backend/schemas/monitor_session.py`
...
### 2.1 Dispatcher 初始化
- [x] 在 `dispatcher.py` 的 `__init__` 中添加 `self._monitor_tasks: dict[int, asyncio.Task] = {}`

### 2.2 Signal File 工具方法
- [x] `_get_monitor_signal_path(self, monitor_session_id: int, cwd: str) -> Path`
  - cwd 使用 `task.last_cwd or task.target_repo or os.getcwd()` fallback（与 loop signal 一致）
- [x] `_read_monitor_signal(self, signal_path: Path) -> dict`

### 2.3 Monitor Session Prompt
- [x] `_build_monitor_session_prompt(self, monitor_session, task) -> str`
  - 包含: 检查次数、监控描述、上下文、signal file 路径
  - 指令: ps aux 检查进程、tail 日志、写 signal file（status: running/done, summary）

### 2.4 Monitor 子进程
- [x] `_run_monitor_subprocess(self, prompt, cwd, model, task_id, monitor_session_id) -> str`
...
- [x] `_run_monitor_session(self, monitor_session, task) -> bool`
...
- [x] `_run_monitor_session_background(self, monitor_session, task_id)`
  - 从 DB 获取 task
...
### 3.1 Loop Prompt 扩展
- [x] 新增 `_get_loop_monitor_hint(self) -> str` 方法
  - 告诉 Claude: 如果启动了后台任务，在 signal file 中设置 `needs_monitor: true` 和 `monitor_context`
  - 包含资源争用提示: 一次迭代只启动一个需要独占资源的任务
- [x] 在 `_build_loop_prompt` 的 **全部 3 个 return 路径** 中追加此 hint（搜索 `return` 语句，分别在 `if iteration == 0 and task.must_complete` / `elif task.must_complete` / `else` 三个分支末尾）

### 3.2 Gate Monitor 插入
- [x] 在 `_run_loop_lifecycle` 中找到 `if action == "continue":` 分支（搜索该字符串定位）:
...
### 4.1 创建 API 文件
- [x] 新建 `backend/api/monitor.py`（项目路由在 `backend/api/` 目录，不是 `backend/routers/`）
...
### 4.2 注册路由
- [x] 在 `backend/main.py` 中 import 并 include monitor router（参考其他 router 的注册方式）

### 4.3 任务取消时清理 Monitor
- [x] 在 `backend/services/task_queue.py` 的 `cancel()` 方法中追加:
  - 批量更新该 task 下所有 running 的 MonitorSession 为 cancelled + completed_at=now()
- [x] 在 `backend/services/task_queue.py` 的 `delete()` 方法中追加（参考 LogEntry 的清理模式）:
  - 先删除该 task 下所有 MonitorCheck（通过 MonitorSession.task_id 关联）
  - 再删除该 task 下所有 MonitorSession
  - 注意: 项目无 FK CASCADE，必须手动清理，否则产生孤儿数据
- [x] 在 `backend/api/tasks.py` 的 `cancel_task` endpoint（搜索 `async def cancel_task`）中追加:
...
### 4.4 服务重启清理
- [x] 在 `dispatcher.py` 的 `_cleanup_stale_state()` 末尾追加:
...
### 5.1 API Client 扩展
- [x] 在 `frontend/src/api/client.ts` 中添加 monitor 相关 API 方法
  - createMonitorSession, deleteMonitorSession, listMonitorSessions, getMonitorChecks
- [x] 添加 TypeScript 接口: MonitorSession, MonitorCheck

### 5.2 Monitor 列表面板组件
- [x] 新建 `frontend/src/components/Chat/MonitorPanel.tsx`
...
- [x] 弹窗表单: 描述(必填)、监控上下文(选填，提供给 monitor 的额外背景信息)、间隔(默认300秒)、最大检查次数(默认100)
- [x] 提交后调用 createMonitorSession API

### 5.4 Monitor 详情页
- [x] 显示检查记录列表（时间倒序）
- [x] 每条记录: 检查编号、时间、状态、摘要、可展开的完整输出
- [x] manual monitor 显示 "5/100 次"，system monitor 显示 "已检查 5 次"

### 5.5 WebSocket 事件处理
- [x] 在 ChatView/LoopChatView 中监听 WebSocket 事件（使用 onMessage 回调模式，不用 lastMessage）:
...
### 6.1 测试
- [x] 测试 MonitorSession / MonitorCheck CRUD（model 层）
- [x] 测试 `_run_monitor_session` 的 done 判断、max_checks 耗尽、取消检测
- [x] 测试 API 权限（manual 可删除，system 不可删除，task_id 归属校验，非 auto 模式拒绝创建，已完成 task 拒绝创建）
- [x] 测试 Loop 集成: needs_monitor → gate → 完成后继续
- [x] 测试子进程失败恢复: subprocess 超时 → 记录 failed MonitorCheck → 继续下一轮
- [x] 测试取消流程: 取消 task → monitor sessions 全部 cancelled
- [x] 测试删除 task: 关联的 MonitorSession 和 MonitorCheck 全部清理，无孤儿数据
- [x] 测试服务重启: running 的 monitor session 被清理为 failed

### 6.2 文档更新
- [x] 更新 CLAUDE.md（如有架构变化）
- [x] 更新 README.md（新增 Monitor 功能说明）
- [x] 更新 TEST.md（新增测试用例）
