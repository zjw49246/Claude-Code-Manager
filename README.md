...
- **交互式提问（ask_user）** — 模型调用内置 `AskUserQuestion`（多选/澄清提问）时，CCM 用 PreToolUse hook 拦截，聊天里弹出 💬 可选卡片（单选/多选/自定义文本），用户选完即把答案喂回模型继续——解决了内置工具在无人值守的 headless/PTY 模式下卡住的问题。CCM 不可达或非托管会话时自动放行原生行为（fail-open）。可用 `ASK_USER_ENABLED=false` 关闭
