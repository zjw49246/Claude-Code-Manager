...
### 2026-07-22 — 自更新任务安全门禁 + 页面非阻塞更新提醒（commit 93253de）
...
### 2026-07-22 — PR #59 审核返修：关闭续跑与停服之间的竞态（commit 6be8dd0）
...
- **验证**：新增确定性 Event/lock 回归覆盖准备→launch 竞态、user/monitor 续跑、无迁移/迁移、手动 pull 快速重启、rollback、最终查询失败、RalphLoop 和手动 Instance 入口；相关后端测试 262 passed、7 deselected，`npx tsc --noEmit` 与前端生产构建通过。独立 8011 服务模拟“手动 pull 后待重启 + executing Task”，强制更新仍返回 409；全量后端测试受本机缺少 `claude_pty` 和既有 Task 排序断言影响未跑绿。用户浏览器实测符合预期，实现提交为 `6be8dd0`。
