 ...
 * 画布(900) 承载最多正文，故遮盖不能太低；配合 applyBgImage 里那层壳色
 * scrim（把图片本身压柔和），文字才浮得出来。卡片(800) 落在画布上，实效
 * 遮盖 = 0.92 + 0.08×0.66 ≈ 0.97，卡片上的文字干净。
 */
const SURFACE_ALPHA: Partial<Record<typeof GRAY_STOPS[number], number>> = {
  '950': 0.80,  // 侧栏壳：导航文字多，调实
  '900': 0.66,  // 画布：透出最多，但要托住正文
  '800': 0.92,  // 卡片：可读性优先，只留一点通透感
  '750': 0.90,
  '700': 0.88,  // 边框/控件底
};
